...
**Ralph Quick Start** (automates this workflow):
```bash
./ralphy.sh                      # Run parallel (default)
./ralphy.sh "add feature"        # Single task
./ralphy.sh --no-parallel        # Sequential mode
```

**Workflows**: `/ralph`, `/setup-ralph`, `/create-branch`, `/commit-fast-conventional`
