...
# ============================================
# Ralphy for Antigravity - Autonomous AI Coding Loop
# Optimized for Antigravity AI with memory bank integration
# Supports: Antigravity (default), Claude Code, OpenCode, Codex, Cursor, Qwen, Droid
# Runs until TASKS.md or PRD is complete
# ============================================
...
# Ralphy config directory (Antigravity-native path)
RALPHY_DIR=".agent/ralph"
...
USE_NOTION=false  # Notion MCP integration for task management

# Runtime options
SKIP_TESTS=false
SKIP_LINT=false
AI_ENGINE="antigravity"  # antigravity (default), claude, opencode, cursor, codex, qwen, or droid
DRY_RUN=false
...
# Parallel execution (enabled by default for Antigravity)
PARALLEL=true
MAX_PARALLEL=3

# PRD source options (TASKS.md is Antigravity native format)
PRD_SOURCE="tasks"  # tasks (TASKS.md), markdown (PRD.md), yaml, github
PRD_FILE="memory-bank/TASKS.md"
GITHUB_REPO=""
GITHUB_LABEL=""
...
run_brownfield_task() {
  ...
  case "$AI_ENGINE" in
    antigravity)
      # Antigravity: Claude CLI with memory bank and agent rules context
      local context_files=""
      [[ -f "$AGENT_RULES_DIR/MEMORY.md" ]] && context_files+="@$AGENT_RULES_DIR/MEMORY.md "
      [[ -f "$MEMORY_BANK_DIR/PRD.md" ]] && context_files+="@$MEMORY_BANK_DIR/PRD.md "
      [[ -f "$MEMORY_BANK_DIR/TSD.md" ]] && context_files+="@$MEMORY_BANK_DIR/TSD.md "
      claude --dangerously-skip-permissions \
        --output-format stream-json \
        -p "$context_files$prompt" 2>&1 | tee "$output_file"
      ;;
    claude)
      claude --dangerously-skip-permissions \
...
show_help() {
  cat << EOF
${BOLD}Ralphy for Antigravity${RESET} - Autonomous AI Coding Loop (v${VERSION})

${BOLD}USAGE:${RESET}
  ./ralphy.sh [options]              # Task mode (uses memory-bank/TASKS.md)
  ./ralphy.sh "task description"     # Single task mode (brownfield)
  ./ralphy.sh --init                 # Initialize .agent/ralph/ config

${BOLD}CONFIG & SETUP:${RESET}
  --init              Initialize .agent/ralph/ with smart defaults
  --config            Show current configuration
  --add-rule "..."    Add a rule to config (e.g., "Always use Zod")

${BOLD}SINGLE TASK MODE:${RESET}
  "task description"  Run a single task without task list (quotes required)
...
${BOLD}PARALLEL EXECUTION:${RESET}
  --parallel          Run independent tasks in parallel (default: on)
...
${BOLD}TASK SOURCE OPTIONS:${RESET}
  --tasks FILE        Use TASKS.md file (default: memory-bank/TASKS.md)
  --prd FILE          Use PRD.md file instead
...
${BOLD}EXAMPLES:${RESET}
  # Antigravity mode (recommended)
  ./ralphy.sh                              # Run with TASKS.md parallel
  ./ralphy.sh "add dark mode toggle"       # Run single task
  ./ralphy.sh --no-parallel                # Sequential execution

  # Alternative engines
  ./ralphy.sh --cursor                     # Use Cursor agent
  ./ralphy.sh --codex                      # Use Codex CLI

  # Advanced workflows
  ./ralphy.sh --branch-per-task --create-pr  # Feature branch + PR workflow
  ./ralphy.sh --max-parallel 5             # Run 5 tasks concurrently
  ./ralphy.sh --prd PRD.md                 # Use traditional PRD.md
  ./ralphy.sh --github owner/repo          # Fetch from GitHub issues

${BOLD}TASK FORMATS:${RESET}
  TASKS.md (Antigravity native):
    - [ ] Task description
    - [/] In progress task
    - [x] Completed task
  ...
  while [[ $# -gt 0 ]]; do
    case $1 in
      ...
      --draft-pr)
        ...
        ;;
      --tasks)
        PRD_FILE="${2:-memory-bank/TASKS.md}"
        PRD_SOURCE="tasks"
        shift 2
        ;;
      --prd)
...
check_requirements() {
  ...
  case "$PRD_SOURCE" in
    tasks)
      # TASKS.md source - Antigravity native format
      if [[ ! -f "$PRD_FILE" ]]; then
        log_warn "$PRD_FILE not found, checking memory-bank/TASKS.md..."
        if [[ -f "memory-bank/TASKS.md" ]]; then
          PRD_FILE="memory-bank/TASKS.md"
          log_info "Using $PRD_FILE"
        else
          log_error "TASKS.md not found in current directory or memory-bank/"
          log_info "Create a TASKS.md file with tasks marked as '- [ ] Task description'"
          log_info "Or use: --prd PRD.md for traditional PRD files"
          exit 1
        fi
      fi
      ;;
  ...
  case "$AI_ENGINE" in
    antigravity)
      # Antigravity Engine Check (Gemini)
      if ! command -v gemini &>/dev/null; then
        log_error "Antigravity Engine (Gemini) not found."
        log_info "Please ensure the Antigravity environment is correctly set up."
        exit 1
      fi
      # Check for memory bank
      if [[ -d "$MEMORY_BANK_DIR" ]]; then
        log_info "Memory bank detected: $MEMORY_BANK_DIR"
      fi
      if [[ -d "$AGENT_RULES_DIR" ]]; then
        log_info "Agent rules detected: $AGENT_RULES_DIR"
      fi
      ;;
    opencode)
    ...
    *)
      ...
      log_info "Supported engines: antigravity, claude, opencode, cursor, codex, qwen, droid"
      exit 1
      ;;
  esac
...
# ============================================
# TASK SOURCES - TASKS.md (Antigravity native format)
# ============================================

# TASKS.md format supports: - [ ], [ ], - [x], [x], - [/] (in progress)
get_tasks_tasksmd() {
  # Match both "- [ ]" and "[ ]" patterns for uncompleted tasks
  grep -E '^\s*-?\s*\[ \]' "$PRD_FILE" 2>/dev/null | \
    sed -E 's/^\s*-?\s*\[ \]\s*//' || true
}

get_next_task_tasksmd() {
  grep -m1 -E '^\s*-?\s*\[ \]' "$PRD_FILE" 2>/dev/null | \
    sed -E 's/^\s*-?\s*\[ \]\s*//' | cut -c1-80 || echo ""
}

count_remaining_tasksmd() {
  grep -c -E '^\s*-?\s*\[ \]' "$PRD_FILE" 2>/dev/null || echo "0"
}

count_completed_tasksmd() {
  grep -c -E '^\s*-?\s*\[x\]' "$PRD_FILE" 2>/dev/null || echo "0"
}

count_inprogress_tasksmd() {
  grep -c -E '^\s*-?\s*\[/\]' "$PRD_FILE" 2>/dev/null || echo "0"
}

mark_task_complete_tasksmd() {
  local task=$1
  local escaped_task
  escaped_task=$(printf '%s\n' "$task" | sed 's/[[\.*^$/]/\\&/g')
  # Handle both "- [ ]" and "[ ]" patterns
  sed -i.bak -E "s/^(\s*-?\s*)\[ \](\s*)${escaped_task}/\1[x]\2${escaped_task}/" "$PRD_FILE"
  rm -f "${PRD_FILE}.bak"
}

mark_task_inprogress_tasksmd() {
  local task=$1
  local escaped_task
  escaped_task=$(printf '%s\n' "$task" | sed 's/[[\.*^$/]/\\&/g')
  sed -i.bak -E "s/^(\s*-?\s*)\[ \](\s*)${escaped_task}/\1[\/]\2${escaped_task}/" "$PRD_FILE"
  rm -f "${PRD_FILE}.bak"
}
...
mark_task_complete() {
  local task=$1
  case "$PRD_SOURCE" in
    tasks) mark_task_complete_tasksmd "$task" ;;
...
run_antigravity_query() {
  local prompt="$1"
  # Run gemini with the prompt non-interactively
  # Filter out known log lines from gemini CLI that might pollute stdout/stderr
  gemini -p "$prompt" 2>&1 | \
    grep -v "Loaded cached credentials" | \
    grep -v "Attempting OAuth discovery" | \
    grep -v "MCP server .* requires authentication" | \
    grep -v "Error during discovery for MCP server" | \
    grep -v "Server .* supports tool updates" | \
    grep -v "Loading extension:" | \
    grep -v "Error executing tool"
}
